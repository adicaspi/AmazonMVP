...
const translations = {
  he: {
    ...
    visitors: "מבקרים",
    clicksBySource: "לחיצות לאמזון לפי מקור",
    clicksBySourceDesc: "כמה לחיצות לאמזון הגיעו מכל מקור תנועה (פייסבוק, אינסטגרם, ישיר, וכו')",
    noClickSourceData: "אין נתונים על לחיצות לפי מקור",
    noClickSourceDataDesc: "ברגע שמישהו ילחץ לאמזון, תראה מאיזה מקור הגיע",
    amazonClicksWord: "לחיצות לאמזון",
  ...
  en: {
    ...
    visitors: "visitors",
    clicksBySource: "Amazon Clicks by Source",
    clicksBySourceDesc: "How many Amazon clicks came from each traffic source (Facebook, Instagram, Direct, etc.)",
    noClickSourceData: "No click source data yet",
    noClickSourceDataDesc: "Once someone clicks to Amazon, you'll see which source they came from",
    amazonClicksWord: "Amazon clicks",
    ...
    <main className={`min-h-screen ${dm.bg} transition-colors duration-300`} dir={isRTL ? "rtl" : "ltr"}>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
              ...
              <div className={`divide-y ${dm.divider}`}>
                {Object.entries(trafficSources)
                  ...
                  .map(([source, count], index) => {
                    ...
                    else if (source === "an") displaySource = "Audience Network";
                    ...
                    const getIcon = () => {
                      if (source === "an") {
                        // Audience Network uses Facebook icon since it's part of Meta
                        return (
        ...
        <section>
          ...
          <p className={`text-sm ${dm.textMuted} mb-4`}>{t.clicksBySourceDesc}</p>

          {Object.keys(clicksBySource).length > 0 ? (
            <div className={`${dm.cardBg} rounded-xl border shadow-sm overflow-hidden transition-colors duration-300`}>
              <div className={`divide-y ${dm.divider}`}>
                {Object.entries(clicksBySource)
                  .sort(([, a], [, b]) => b - a)
                  .map(([source, count], index) => {
                    ...
                    // Get icon for source
                    const getSourceIcon = () => {
                      if (source === "an" || source === "fb" || source.includes("facebook")) {
                        return (
                          <svg className="w-5 h-5" viewBox="0 0 24 24" fill="#1877F2">
                            <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                          </svg>
                      ...
                      <div key={source} className={`p-4 ${dm.tableHover} transition`}>
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-2">
                            {getSourceIcon()}
                            {isTop && (
                              <span className={`text-xs px-2 py-0.5 rounded font-medium ${darkMode ? "bg-orange-900/50 text-orange-300" : "bg-orange-100 text-orange-700"}`}>
                                #1
                              </span>
                            )}
                            <span className={`font-medium ${dm.text}`}>{displaySource}</span>
                          ...
                          <div className={isRTL ? "text-left" : "text-right"}>
                            <div className={`font-bold ${dm.text}`}>{count}</div>
                            <div className={`text-xs ${dm.textMuted}`}>{percentage.toFixed(1)}% {t.ofAllClicks}</div>
                          </div>
                        </div>
                        <div className={`h-2 ${dm.barBg} rounded-full overflow-hidden`}>
                          <div
            ...
            <div className={`${dm.cardBg} rounded-xl border p-8 text-center transition-colors duration-300`}>
              <div className={`${dm.textLight} text-4xl mb-3`}>🎯</div>
              <p className={dm.textMuted}>{t.noClickSourceData}</p>
              <p className={`text-sm ${dm.textLight} mt-1`}>{t.noClickSourceDataDesc}</p>
            </div>
          )}
        </section>
