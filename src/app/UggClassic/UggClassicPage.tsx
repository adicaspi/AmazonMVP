...
import { SizeFinder, SizeProvider, HeroSizeCta } from "../UggScuffette/SizeFinder";
import type { AmazonProductData } from "@/lib/amazon-creators-api";

// /UggClassic — the CLASSIC-design UGG Scuffette II bridge page (same CRO
// structure as GrandeLash/NewBalance), kept alongside the editorial
// /UggScuffette as an A/B pair. Identical size-selection behavior: shared
// SizeProvider, size pills in the hero + a Find-your-size section, and the
// same qualified-click gate (AmazonClick only after a size was chosen).
// The generic-slippers comparison section is dropped by owner preference.
// ── Product constants ─────────────────────────────────────────────
// Owner's Facebook-campaign SiteStripe link (tag=ugg-fb-20) — same link as
// /UggScuffette for now; send a separate SiteStripe tag to split the two
// pages in Associates. DIRECT amazon.com URL only (NEVER amzn.to).
...
export function UggClassicPage({ trackingPage, amazonLink, product }: { trackingPage: string; amazonLink?: string; product?: AmazonProductData | null }) {
  ...
  return (
    ...
    <div className="min-h-screen bg-white">
          ...
          <div className="grid gap-6 md:gap-10 items-center md:items-stretch md:grid-cols-[minmax(0,7fr)_minmax(0,5fr)]">
            <div className="order-2 text-center md:text-left md:flex md:flex-col">
              ...
              <div className="max-w-md mx-auto md:mx-0 md:mt-auto">
                ...
                {/* Size pills + size-aware CTA — same shared size state and
                    qualified-click behavior as /UggScuffette */}
                <HeroSizeCta
                  amazonLink={AMAZON_LINK}
                  priceValue={priceValue}
                  idleLabel="See Today's Price on Amazon"
                  buttonClassName="flex items-center justify-center gap-3 w-full px-6 py-4 bg-gradient-to-r from-amber-700 to-amber-800 hover:from-amber-800 hover:to-amber-900 text-white font-bold text-lg rounded-2xl transition-all duration-200 shadow-2xl shadow-amber-900/30 hover:scale-[1.02] active:scale-[0.98]"
                />
      ...
      {/* Find your size — same shared state as the hero pills */}
      <section className="py-10 md:py-14 bg-white">
        <div className="max-w-2xl mx-auto px-4">
          <SizeFinder
            amazonLink={AMAZON_LINK}
            priceValue={priceValue}
            buttonClassName="inline-flex items-center justify-center gap-2 w-full sm:w-auto px-8 py-3.5 bg-amber-700 hover:bg-amber-800 text-white font-bold rounded-full transition-all shadow-lg"
          />
        </div>
      </section>
